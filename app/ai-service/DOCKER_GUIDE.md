# Docker Deployment Guide for Aegis AI Service

This guide provides comprehensive instructions for deploying the Aegis AI Service using Docker with CUDA support.
...
### 1. Development Stage
```bash
docker build -t aegis-ai-service:dev --target development .
```
...
```bash
docker build -t aegis-ai-service:latest --target production .
```
...
```bash
docker build -t aegis-ai-service:gpu --target production-gpu .
```
...
1. **Build the image:**
```bash
docker build -t aegis-ai-service:latest .
```
...
- Container runs as non-root user `aegis`
...
### Image Updates
```bash
docker pull aegis-ai-service:latest
