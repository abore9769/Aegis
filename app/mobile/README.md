...
3. Build a development client or production build so the custom `aegis://` scheme is registered on the device.
...
- `aegis://` is configured as the mobile deep-link scheme so the wallet can return the user to Aegis after approval.
